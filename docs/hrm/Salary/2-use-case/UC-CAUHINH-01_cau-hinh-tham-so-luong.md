...
2. Một bản ghi lịch sử được thêm: người sửa, thời điểm, tham số nào, giá trị cũ → giá trị mới.
3. Kỳ dựng hoặc điều chỉnh sau thời điểm này dùng tham số mới.
4. Kỳ đã chốt giữ nguyên snapshot tham số lúc chốt.
...
6. Hệ thống lưu bộ tham số mới và ghi một bản ghi lịch sử thay đổi (người sửa, thời điểm, giá trị cũ → mới cho từng tham số đã đổi).
...
- **UC-CAUHINH-01.AC.2** — Sau khi lưu, nếu quản lý mở "Lịch sử tham số", hệ thống hiển thị danh sách các lần đổi (mới nhất trước), chỉ đọc.
...
3. Chỉ tài khoản quản trị được định danh mới sửa được tham số (luật Firestore chặn tài khoản khác).
4. Lịch sử tham số chỉ được đọc và thêm mới.

## Notes and Issues
- Lưu lịch sử các lần đổi tham số lương: **có** (đã chốt 08/09/2026).
