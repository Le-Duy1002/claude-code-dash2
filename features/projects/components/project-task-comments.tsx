...
function Composer({
  ...
  async function submit() {
    ...
    try {
      ...
      toast.error("Không gửi được bình luận. Thử lại.")
    } finally {
      setPending(false)
    }
  }
    ...
    <AlertDialog>
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            Bình luận này và toàn bộ câu trả lời bên trong sẽ bị xoá vĩnh viễn.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
      ...
      <div className="flex flex-col gap-1 py-3">
        ...
        <div className="-ml-2 flex items-center gap-1">
          ...
          {node.createdByUid === currentUid ? (
            <DeleteButton
              onConfirm={() =>
                deleteProjectTaskCommentThread(taskId, [
                  node.id,
...
export function ProjectTaskCommentsDialog({
  task,
  open,
  onOpenChange,
}: {
  ...
  React.useEffect(() => {
    ...
    setLoading(true)
    setComments([])
    const unsub = subscribeToProjectTaskComments(
      taskId,
      (next) => {
        setComments(next)
        setLoading(false)
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        ...
        {user && taskId ? (
          <Composer
            onSubmit={(content) =>
              addProjectTaskComment(taskId, { content, parentId: null, user })
            }
          />
        ) : (
